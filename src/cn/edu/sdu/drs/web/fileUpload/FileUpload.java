package cn.edu.sdu.drs.web.fileUpload;
 ...
 * 类名：FileUpload
 * <p>
 * 作用：使用common实现文件的上传功能 并得到上传文件的title，keywords，kind，describe，url等信息
 * <p>
 * @author join
 */
public class FileUpload extends javax.servlet.http.HttpServlet implements
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		// boolean isMultipart =
		// ServletFileUpload.isMultipartContent((HttpServletRequest) new
		...
		try {
			...
			List listDescribe = new ArrayList();
			...
			while (iter.hasNext()) {
				...
				if (!item.isFormField()) {
					...
					// 设置文件的访问路径
					String url = "http://" + ipLocal + ":8080/resources/"
							+ newFileName;
					listUrl.add(url);
			...
			req.getRequestDispatcher("/Manager/showFileAddContent.jsp")
					.forward(req, resp);
	...
	/**
	 * 函数名：getPath 作 用：得到tomcat根目录下面某个文件夹的路径 参 数：出递过来的tomcat根目录下面的某个文件夹 返回值：字符串
	 */
