...
public class InitSystem extends BaseController{
    ...
    public void initTenant() {
        ...
        t1.setPort(8081);
        t1.setRootFolder("cjbs-folder");

        Tenant t2 = new Tenant("好学网");
        t2.setNote("积累知识的网站，增长智慧的源泉！");
        t2.setHost("127.0.0.1");
        t2.setPort(8082);
        t2.setRootFolder("hxw-folder");

        Tenant t3 = new Tenant("test");
        t3.setNote("test");
        t3.setHost("127.0.0.1");
        t3.setPort(8081);
        t3.setRootFolder("test-folder");
    ...
    public void initAdmin4HXW(String tenantName) {
        /* ----------------------admin----------------------------- */
        Admin a = new Admin("user.admin.xhw");
        a.setRealname("yonggangYuan - HXW");
        ...
        Admin b = new Admin("resource.admin.hxw");
        b.setRealname("shaozeMi - HXW");
        ...
        /* ----------------------rootAdmin----------------------------- */
        RootAdmin ra = new RootAdmin("root.hxw", "root.hxw");
        ra.setRealName("root.hxw");
     ...
     * 初始化管理员模块
     */
    @RequestMapping("/init.admin.CJBS")
    public void initAdmin4CJBS(String tenantName) {
        /* ----------------------admin----------------------------- */
        Admin a = new Admin("user.admin.cjbs");
        a.setRealname("yonggangYuan - CJBS");
        a.setEmail("sdcsyyg@163.com");
        a.setPassword(a.getId());
        a.setPhone("18063420612");
        QueryResult<PrivilegeGroup> qus = privilegeGroupService.getScrollData(PrivilegeGroup.class, "o.name=?1", new Object[]{"用户服务组"});
        ...
        Admin b = new Admin("resource.admin.cjbs");
        b.setRealname("shaozeMi - CJBS");
        b.setEmail("sdumsz@163.com");
        b.setPassword(b.getId());
        b.setPhone("18063420630");
        QueryResult<PrivilegeGroup> qusb = privilegeGroupService.getScrollData(PrivilegeGroup.class, "o.name=?1", new Object[]{"资源服务组"});
        ...
        /* ----------------------rootAdmin----------------------------- */
        RootAdmin ra = new RootAdmin("root.cjbs", "root.cjbs");
        ra.setRealName("root.cjbs");
        QueryResult<PrivilegeGroup> qusp = privilegeGroupService.getScrollData(PrivilegeGroup.class, "o.name=?1", new Object[]{"权限服务组"});
        QueryResult<PrivilegeGroup> qusr = privilegeGroupService.getScrollData(PrivilegeGroup.class, "o.name=?1", new Object[]{"管理员服务组"});
        PrivilegeGroup privilegeGroups4Userr = null, pg = null;
        if(qusp != null && qusp.getResultList().size() > 0){
    ...
    public void initUser() {
        ...
        QueryResult<Tenant> qt =  tenantService.getScrollData(Tenant.class, "o.name=?1", new Object[]{"好学网"});
        Tenant t = null;
        if(qt !=null && qt.getResultList().size() > 0){
            t = qt.getResultList().get(0);
        }
        QueryResult<Tenant> qt1 =  tenantService.getScrollData(Tenant.class, "o.name=?1", new Object[]{"长江报社"});
        Tenant t1 = null;
        if(qt1 !=null && qt1.getResultList().size() > 0){
            t1 = qt1.getResultList().get(0);
        }
        for (int i = 0; i < 10; i++) {
